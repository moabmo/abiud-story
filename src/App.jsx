...
export default function App() {
  ...
  return (
    <main className="min-h-screen bg-[#f7f9fc] text-slate-900">
      <button onClick={() => { localStorage.removeItem("abiud-auth"); setIsAuthenticated(false); }} className="fixed right-5 top-5 z-50 rounded-full bg-white px-4 py-2 text-sm font-semibold text-[#064b96] shadow-md">
        Logout
      </button>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-[#002b5c] via-[#064b96] to-[#0b6bc2] text-white">
        <div className="mx-auto grid max-w-7xl gap-10 px-6 py-24 md:grid-cols-2 md:px-10">
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }}>
            ...
            <a href="#story" className="mt-8 inline-flex items-center gap-2 rounded-full bg-[#ffd200] px-5 py-3 font-semibold text-[#002b5c]">
      ...
      {selectedChapter && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-950/60 px-6 backdrop-blur-sm">
          <div className="relative max-h-[85vh] w-full max-w-2xl overflow-y-auto rounded-[2rem] bg-white p-8 shadow-2xl">
            <button onClick={() => setSelectedChapter(null)} className="absolute right-5 top-5 rounded-full bg-slate-100 p-2 text-slate-700 hover:bg-slate-200">
              <X size={20} />
            </button>
            ...
            <div className="mt-8 rounded-3xl bg-[#f7f9fc] p-6">
              <h3 className="text-lg font-bold text-slate-900">Notes to add later</h3>
            ...
            {selectedChapter.title === "Simping Chats" && (
              <div className="mt-8 space-y-4">
                <div className="rounded-3xl border border-pink-200 bg-pink-50 p-5">
                  <p className="text-sm font-bold text-pink-700">Woman 1</p>
                  <p className="mt-2 text-slate-700">“I kept trying to explain myself hoping you would finally understand me.”</p>
                </div>

                <div className="rounded-3xl border border-blue-200 bg-blue-50 p-5">
                  <p className="text-sm font-bold text-blue-700">Woman 2</p>
                  <p className="mt-2 text-slate-700">“I gave too much emotionally until I no longer recognized myself.”</p>
                </div>

                <div className="rounded-3xl border border-red-200 bg-red-50 p-5">
                  <p className="text-sm font-bold text-red-700">Emotional State</p>
                  <p className="mt-2 font-semibold text-red-600">A desperate season of over-caring, over-texting, and losing emotional balance.</p>
                </div>

                <div className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
                  <p className="mb-4 text-sm font-bold text-slate-500">Sample Simping Chat — Woman 1</p>

                  <div className="space-y-3">
                    <div className="ml-auto max-w-[80%] rounded-3xl rounded-br-md bg-[#064b96] px-4 py-3 text-white">
                      <p className="text-xs font-bold text-white/70">Monyoro</p>
                      <p>Please talk to me. I honestly do not like how distant we have become.</p>
                    </div>

                    <div className="ml-auto max-w-[80%] rounded-3xl rounded-br-md bg-[#064b96] px-4 py-3 text-white">
                      <p className="text-xs font-bold text-white/70">Monyoro</p>
                      <p>I care about you more than I probably should.</p>
                    </div>

                    <div className="mr-auto max-w-[70%] rounded-3xl rounded-bl-md bg-slate-100 px-4 py-3 text-slate-700">
                      <p className="text-xs font-bold text-slate-400">Woman 1</p>
                      <p>Ok.</p>
                    </div>

                    <div className="ml-auto max-w-[80%] rounded-3xl rounded-br-md bg-[#064b96] px-4 py-3 text-white">
                      <p className="text-xs font-bold text-white/70">Monyoro</p>
                      <p>I love you, and I hate that I keep saying it when you barely respond.</p>
                    </div>
                  </div>
                </div>

                <div className="rounded-[2rem] border border-slate-200 bg-white p-5 shadow-sm">
                  <p className="mb-4 text-sm font-bold text-slate-500">Sample Simping Chat — Woman 2</p>

                  <div className="space-y-3">
                    <div className="ml-auto max-w-[80%] rounded-3xl rounded-br-md bg-[#00843d] px-4 py-3 text-white">
                      <p className="text-xs font-bold text-white/70">Monyoro</p>
                      <p>I have typed and deleted this message so many times.</p>
                    </div>

                    <div className="ml-auto max-w-[80%] rounded-3xl rounded-br-md bg-[#00843d] px-4 py-3 text-white">
                      <p className="text-xs font-bold text-white/70">Monyoro</p>
                      <p>I just wanted to matter to you the way you mattered to me.</p>
                    </div>

                    <div className="mr-auto max-w-[70%] rounded-3xl rounded-bl-md bg-slate-100 px-4 py-3 text-slate-700">
                      <p className="text-xs font-bold text-slate-400">Woman 2</p>
                      <p>Okay.</p>
                    </div>
                  </div>
                </div>
